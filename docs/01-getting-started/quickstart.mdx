...
### 2. Login to Nblocks Admin and copy your app's signup link.
Open a browser tab and login to [Nblocks Admin](https://admin.nblocks.cloud).

Nblocks has created a nice set of default data like app roles and payment plans that we will use in this test. Go ahead and have a look around.
If you click *Workspaces* you'll see that no one has signed up for your app yet. This is what we'll be changing now.

Click *Authentication* and choose the *Signup* tab. Click *Signup link without plan* to copy the link to your clipboard.

![Signup links](../assets/dev-onboarding/signup-links.png)

### 3. Use the link to sign up as a new user to your app
The link you've copied is a unique sign up link for new users to sign up to your app.

Open a new browser tab and paste the link into the address bar and hit enter.
...
Open the console and you should see the full access token that we're printing from the component.
