...
export default function ProtectedContent({roles, privileges, children}) {
  ...
  useEffect(() => {
    ...
    if (accessToken) {
      ...
      setGranted(hasRole(decoded) || hasPrivilege(decoded));
    }
  }, []);

  // Helper method to see if the users token contains any of the roles 
  const hasRole = (decoded) => {
    return roles ? roles.includes(decoded.role) : false;
  }

  // Helper method to see if the users token contains any of the privileges 
  const hasPrivilege = (decoded) => {
    return privileges ? privileges.some(scope => decoded.scope.includes(scope)) : false;
  }
