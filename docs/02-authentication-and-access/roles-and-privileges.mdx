...
Nblocks automatically assigns the `OWNER` role to the person who initiates the creation of a new tenant. This person becomes the first user and responsible owner of that tenant.

:::

## Define your roles

In [Nblocks Admin](https://admin.nblocks.cloud), begin by creating a privilege and adding an optional description:  
...
After protecting your [backend](/docs/authentication-and-access/protect-backend) or [frontend](/docs/authentication-and-access/protect-frontend), your newly defined roles will become available for granting user access, as reflected in the updated [access token](/docs/authentication-and-access/protect-backend#short-info-about-the-access-token):
