...
* The `tid` property shows the id of the tenant / workspace
...
const jwtMiddleware = async (req, res, next) => {
  if (req.headers['authorization']) {
    try {
      ...
      // Deconstruct user from the verified token
      // And store the decoded information in a variable that can be used by other middlewares
