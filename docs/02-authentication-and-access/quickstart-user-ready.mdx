...
Create a new file `/app/auth/login.jsx`
...
Create a new file `/app/auth/oauth-callback.jsx`
...
// Users will get back to this page after finishing login
...
function ProtectedRoute<T>(
  ...
  args: { roles?: string[]; privileges?: string[]; redirectTo?: string }
) {
  return (props: T) => {
    // This is the path which users without access should be redirect to
...
## User list
In the last last part of the quickstart we're adding a user management view to let the users manage and invite others to their workspace.
