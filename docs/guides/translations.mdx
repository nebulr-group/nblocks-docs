...
## Frontend
Our frontend plugins comes with i18n support and during the installation process some translation files where put into your project. 
The NBlocks views will load these files over HTTP. Feel free to change these text entries to your own standards. 
You can add more languages by adding a new file named `nblocks_${LANG}.json`, where `LANG` should be a lower case in ISO_639-1 format and using the same translation keys as the other files.
...
Even if you're supporting different languages in your app, the NBlocks platform (responsible for emailing, onboarding etc.) supports these language at the time.
