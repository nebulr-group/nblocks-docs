...
# Users & Tenants
Everytime we've started a new SaaS project before we've seen the following needs for security and user management:
...
* Users needs to be able to login and be granted a set of predefined privilegues.
...
* Same user might be granted different privilegues for different workspaces.
...
The tenant contains information about the current subscription plan which can be used to render a specific feature set and other fields like locale which can be used to render translate resources. 
Other data could be files in the file storage etc. Many calls that you make to Nblocks requires you to provide an identification for the tenant context for this purpose.

Tenants gets created automatically in Nblocks if you've opt in for our [checkout solution](stripe). 
...
## Users
A user represents an individual and he's/her's access to specific tenants and therefore your application. The same user can be added to several tenants and still use the same credentials as before. After login, the user will be able to select what tenant to access.
...
| onboarded         | If user is onboarded or not (should be put through onboarding after first login) |
| enabled           | If user should be allowed to login or not. Changeble via user management view |
...
1. The new tenant ower user will receive an email containing an onboarding link to your app.
1. The onboarding link will guide the user through a initial set password process or redirect straight to login if the user already existed in another tenant.
1. After login, each subsequent call the user makes to your api will contain resolved security data about a new tenant and its user. It's now up to you to render data and a response to these request.
