...
With this module we also have you covered from customers that stops paying. Users of these tenants will be locked out if they neglect the Nblocks reminders.
...
Naturally, different plans sets out how rich feature set your app will provision. The richer feature set, the higher pricing.
...
:::note
Everytime a customer (may it be a new or an existing one) checkout a new subscription; a new tenant with this plan is created. 
You're app will be able to easily obtain the plan information everytime a request to your backend is made. 
Using that information you'll be able to provision different feature sets
:::

Once we've define the different types of plans it's time to think of the pricing.
...
## Taxes
Many businesses are obligated to collect Value added tax (VAT) from their domestic customers. This may vary if you have internation customers. 
With that in mind we've done it easy to apply taxes like this for the regions you decide.

Let's say we're a French startup and we'd like our French customers to get a VAT of 20 % added to the total whilst having our English customers to proceed without affecting the total.
We therefor declare these taxes:
...
## Checkout
We offer a prebuilt checkout solution that you can easily embedd on your own product web page. 
The checkout allows new customers to subscripe to a plan with their credit card that automatically creates a new tenant workspace and user access to your application. 
...
1. Provide your Stripe api credentials obtained from your Stripe dashboard to Nblocks with which Nblocks will authenticate itself with.
1. Then we update the business model which will synchronize automatically with Stripe creating the nessesary resources in Stripe.
1. In the last example we use the checkout api to obtain an checkout id to use with Stripes official frontend library to open the prebuilt checkout view.
...
:::warning
Don't change products and price data inside Stripe created automatically by Nblocks. This might affect the checkout availablity and recurring subscription management.
:::
