...
1. First we make sure to provide our Stripe api credentials with which Nblocks will authenticate itself with.
...
...
const checkoutSession = await client.createCheckoutSession({region: "FR"});
```
