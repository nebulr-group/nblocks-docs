...
Nblocks assigns the `OWNER` role to the person that iniated the creation of a [tenant](tenants-users). That person will be the first user and the responsible owner of that tenant.
