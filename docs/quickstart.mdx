...
This is achived using the `resourceMappings.json` file which is read by the plugin during startup. 
:::note
Every time you change `resourceMappings.json`, a server restart is necessary for the changes to take effect.
:::
**Let's proceed!** 👨‍💻
1. 
Make all graphql endpoints require the `USER_READ` privilegue by adding this to the file and restart your backend server. 
...
You've just unlocked the next level you're ready take on next challenge.
