...
### Frontend
It's time for the frontend! Install our plugin contacining ready-to-use views that make use of your Nblocks loaded backend.
:::tip Don't have a project yet?
...
Wouldn’t it be cool if your logo was used in both the UI and the onboarding email you just received?
Let’s make it happen and while we’re at it let’s try changing some colors and texts in the UI so you get the hang of it 😃

Here's an overview of the stuff we're changing here: 
...
But hold on! We've made it super simple for this quickstart and all you need to do is the following two step:
...
This, and other translation files, were put here by the setup script and defines the text content of all Nblocks views.
...
💥 Perfect! You just completed your first branding. Click [http://localhost:8100/auth/set-password/token](http://localhost:8100/auth/set-password/token) to reveal your changes.   
...
If you haven’t already don’t forget to join our [Discord](https://discord.gg/WgzfZrCP) to hit us up with questions, announcements and updates.
