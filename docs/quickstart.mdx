...
The installation instructions below requires you to run the commands inside your project root folders and not anywhere else.
...
1. Step into your NestJS project and install the nblocks-nestjs plugin.
```bash
...
1. Step into your Ionic project and run the scaffolding script to install our nblocks-ionic plugin and all it's dependencies.
```bash
...
While we're at it let's also try changing some colors and texts so you get a hang of it 😃
...
Change both the `logo` and `websiteUrl` to anything that is publicly available on the internet. Don't worry, this is just for demoing.
...
:::tip
The logo URL should be a publicly available image on the internet. Do you have a web page with your own logo? 
Go ahead and add it, aswell as the url to the actual web page.
:::
...
3. Lastly we're not keen about the default Nblocks primary color for buttons and links and we wish to change it. 
...
💥 Perfect! You just did your first branding. We'll wait to unveal the results until you get to the next final step where you'll add a new user.
...
5. Remember that we customized the branding a bit? You're set-password page should look something like this now! 🙀
<img
...
- Read more about what's possible with [custom branding](articles/branding.mdx).
- Get ready and [secure your app](articles/security.mdx).
- Understand the concept of [Tenants and Users](articles/tenants-users.mdx).
- Go full hard core and checkout the [API client](articles/api-client.mdx) and [API reference](https://nebulr-group.github.io/nblocks-api-docs).
